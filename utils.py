...
import data_config
from datasets.CD_dataset import CDDataset, xBDataset, xBDatasetMulti, TestInput, CC


def get_loader(data_name,args, img_size=1024, batch_size=8, split='test',
               is_train=False, dataset='construction_changes', patch=None):
    dataConfig = data_config.DataConfig().get_data_config(data_name)
    root_dir =  args.root + dataConfig.root_dir
    label_transform = dataConfig.label_transform

    if dataset == 'CDDataset':
        #print('###################################################################### batti dangay karyengay kia!!!!!!!!!!!!!')
    ...
    elif dataset == 'construction_changes':
        data_set = CC(root_dir=root_dir, split=split,
                                  img_size=img_size, is_train=is_train,
                                  label_transform=label_transform)

    elif dataset == 'TestInput':
        data_set = TestInput(root_dir=root_dir, split=split,
                                  img_size=img_size, is_train=is_train,
                                  label_transform=label_transform)
    else:
...
def get_loaders(args):
    ...
    elif args.dataset == 'construction_changes':
        training_set = CC(root_dir=root_dir, split=split,
                                 img_size=args.img_size,is_train=True,
                                 label_transform=label_transform)
        val_set = CC(root_dir=root_dir, split=split_val,
                                 img_size=args.img_size,is_train=False,
                                 label_transform=label_transform)

...
def focal_loss(
    ...
    print(f'weight of the loss {weight.shape}')
...
def focal_loss_xBD(pred, true):
    ...
    print('hehehehehhehehehehehehehehehehheehhhhhhhhhhhhhhhhhh')
    return loss0 * 0.01 + loss1 * 0.01 + loss2 * 0.7 + loss3 * 0.7 + loss4 * 0.01

def focal_loss_xCC(pred, true):
    B, C, H, W = pred.shape
    true = true.squeeze()
    msk0 = torch.zeros([B, H, W]).cuda()
    msk1 = torch.zeros([B, H, W]).cuda()
    ...
    #loss4 = focal_loss2D(pred[:, 4, :, :], msk4)
    print('hehehehehhehehehehehehehehehehheehhhhhhhhhhhhhhhhhh')
    return loss0 * 0.01 + loss1 * 0.1 + loss2 * 0.1 + loss3 * 3

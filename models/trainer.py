...
from models.losses_modified import cross_entropy
...
class CDTrainer():
    ...
    def _forward_pass(self, batch):
        ...
        print(img_in1.shape, img_in2.shape)
        self.G_pred = self.net_G(img_in1, img_in2)
        self.G_final_pred = self.G_pred
        print('OUtput shape',self.G_final_pred.shape)
    ...
    def _backward_G(self):
        ...
        if gt.shape[0] != 1:
            ...
            print(self.G_pred.shape, '____________', gt.shape)
    ...
    #     print(f'Ground truth shape {gt.shape}, {self.G_pred.shape}')
    #     i         = 0
    #     temp_loss = 0.0
    #     self.weights = [0.5, 0.5, 0.5, 0.8]
    #     for pred in self.G_pred:
    #         if pred.size(2) != gt.size(2):
    #             print(pred.size(), gt.size())
    #             gt = F.interpolate(gt, size=(pred.shape[-1],pred.shape[-1]))
    #             print(gt.size())
